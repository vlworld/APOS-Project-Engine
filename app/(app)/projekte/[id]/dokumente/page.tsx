...
export default function DokumentePage() {
  ...
  async function load() {
    const res = await fetch(`/api/projekte/${projectId}/dokumente`);
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{documents.length} Dokumente</p>
        </div>
        <button
        ...
        <div className="bg-white border border-blue-200 rounded-xl px-4 py-4 mb-4 space-y-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={saving || !form.title.trim() || !form.fileName.trim() || !form.fileUrl.trim()}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-base font-semibold text-gray-900 mb-1">Noch keine Dokumente</h3>
          <p className="text-sm text-gray-500 max-w-xs mb-5">Verknüpfen Sie Projektdokumente per URL.</p>
              ...
              {filtered.map((d) => (
                <tr key={d.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    {d.category ? (
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${CATEGORY_STYLES[d.category] || "bg-gray-100 text-gray-600"}`}>
                        {d.category}
                      </span>
                    ) : (
