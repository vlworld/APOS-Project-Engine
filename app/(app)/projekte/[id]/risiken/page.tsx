...
export default function RisikenPage() {
  ...
  async function load() {
    const res = await fetch(`/api/projekte/${projectId}/risiken`);
  ...
  for (const p of LEVEL_OPTIONS) {
    ...
    for (const i of LEVEL_OPTIONS) {
      matrixData[p][i] = riskItems.filter(
            ...
            {items.map((r) => (
              <tr key={r.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                ...
                <td className="px-3 py-2.5 text-xs text-gray-600">
                  {LEVEL_LABELS[r.probability || ""] || r.probability || "—"}
                </td>
                <td className="px-3 py-2.5 text-xs text-gray-600">
                  {LEVEL_LABELS[r.impact || ""] || r.impact || "—"}
                </td>
                <td className="px-3 py-2.5">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_STYLES[r.status] || "bg-gray-100 text-gray-600"}`}>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Risiken & Issues</h2>
          <p className="text-sm text-gray-500 mt-0.5">{riskItems.length} Risiken, {issueItems.length} Issues</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => startCreate("RISK")}
          ...
          >
            <Plus size={15} /> Neues Risiko
          </button>
          <button
            onClick={() => startCreate("ISSUE")}
      ...
      {creatingType && (
        <div className="bg-white border border-blue-200 rounded-xl px-4 py-4 mb-4 space-y-3">
          <p className="text-xs font-semibold text-gray-500 uppercase">
            {creatingType === "RISK" ? "Neues Risiko" : "Neues Issue"}
          </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setCreatingType(null)} className="text-gray-400 hover:text-gray-600">
              <X size={16} />
            </button>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-base font-semibold text-gray-900 mb-1">Noch keine Risiken oder Issues</h3>
          <p className="text-sm text-gray-500 max-w-xs mb-5">Erfassen und verfolgen Sie Projektrisiken.</p>
          <div className="flex items-center gap-2">
            <button onClick={() => startCreate("RISK")} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg">
              <Plus size={15} /> Neues Risiko
            </button>
            <button onClick={() => startCreate("ISSUE")} className="flex items-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium px-4 py-2 rounded-lg">
              <Plus size={15} /> Neues Issue
            </button>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          {riskItems.length > 0 && (
            <div>
              ...
              <div className="inline-block bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex items-end gap-1">
                  ...
                  <div>
                    {[...LEVEL_OPTIONS].reverse().map((prob) => (
                      <div key={prob} className="flex items-center gap-1 mb-1">
                        ...
                        {LEVEL_OPTIONS.map((imp) => (
                          <div
                            key={imp}
                            className={`w-14 h-14 rounded-lg flex items-center justify-center text-sm font-bold ${matrixColors[prob][imp]} ${matrixData[prob][imp] > 0 ? "text-gray-800" : "text-gray-300"}`}
                          >
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      {LEVEL_OPTIONS.map((imp) => (
                        <span key={imp} className="w-14 text-center text-[10px] text-gray-500">{LEVEL_LABELS[imp]}</span>
                      ))}
                    </div>
                    <div className="text-center mt-1">
                      <span className="text-[10px] text-gray-400">Auswirkung</span>
                    </div>
                  </div>
                </div>
              </div>
