...
export default function VobPage() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/projekte/${projectId}/vob`);
  ...
  const handleDelete = async (vobId: string) => {
    await fetch(`/api/projekte/${projectId}/vob/${vobId}`, { method: "DELETE" });
    await fetchItems();
  };

  const handleStatusChange = async (vobId: string, status: string) => {
    await fetch(`/api/projekte/${projectId}/vob/${vobId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        status,
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Fristen, Behinderungen, Nachträge, Abnahmen & Mängel</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-gray-500">
            Es wurden noch keine Einträge für {tabMeta.label} angelegt.
          </p>
        </div>
      ) : (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {(isDueSoon(item.dueDate) || isOverdue(item.dueDate)) && item.status !== "RESOLVED" && (
                          <Calendar
                            className={`w-3.5 h-3.5 ${isOverdue(item.dueDate) ? "text-red-500" : "text-amber-500"}`}
                          />
                        )}
                        <span className={
                          item.status !== "RESOLVED" && isOverdue(item.dueDate)
                            ? "text-red-600 font-medium"
                            : item.status !== "RESOLVED" && isDueSoon(item.dueDate)
