...
import {
  Plus, Loader2, X, Package, Truck, Calendar,
} from "lucide-react";
...
export default function BeschaffungPage() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/projekte/${projectId}/beschaffung`);
  ...
  const handleDelete = async (procId: string) => {
    await fetch(`/api/projekte/${projectId}/beschaffung/${procId}`, { method: "DELETE" });
    await fetchItems();
  };
  ...
  const handleStatusChange = async (procId: string, status: string) => {
    ...
    if (status === "DELIVERED") {
      patch.actualDelivery = new Date().toISOString();
    }
    await fetch(`/api/projekte/${projectId}/beschaffung/${procId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Material- & Lieferantenübersicht</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="md:col-span-2 lg:col-span-1">
              ...
              <input
                ...
                placeholder="z.B. Bewehrungsstahl"
              />
            </div>
            <div>
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            <button
              ...
              disabled={saving || !formMaterial.trim()}
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-gray-500">
            Erstellen Sie die erste Bestellung für dieses Projekt.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-500">Geliefert</th>
              ...
              {items.map((item) => {
                ...
                return (
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-gray-700">{fmt(item.actualDelivery)}</td>
                    <td className="px-4 py-3">
