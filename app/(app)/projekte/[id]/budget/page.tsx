...
import {
  Plus, Loader2, X, TrendingUp, TrendingDown, Wallet, BarChart3, Receipt,
} from "lucide-react";
...
export default function BudgetPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/projekte/${projectId}/budget`);
    if (res.ok) {
  ...
  const handleDelete = async (budgetId: string) => {
    await fetch(`/api/projekte/${projectId}/budget/${budgetId}`, { method: "DELETE" });
    await fetchData();
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Soll-Ist-Vergleich & Kostenübersicht</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className={`border rounded-2xl p-5 ${vc.bg} ${vc.border}`}>
          ...
          <div className="flex items-center gap-2">
            {summary.variance <= 0 ? (
              <TrendingDown className={`w-5 h-5 ${vc.text}`} />
            ) : (
              <TrendingUp className={`w-5 h-5 ${vc.text}`} />
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="z.B. Rohbau, Elektro"
              />
            </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-gray-500">
            Erstellen Sie die erste Budgetposition für dieses Projekt.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
                  ...
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`font-medium ${
                        rv <= 0 ? "text-emerald-600" : rv <= 10 ? "text-amber-600" : "text-red-600"
                      }`}>
                        {fmtPercent(rv)}
                      </span>
                    </td>
              ...
              <tr className="bg-gray-50 font-semibold border-t border-gray-200">
                ...
                <td className="px-4 py-3 text-right">
                  <span className={`${
                    totalVar <= 0 ? "text-emerald-600" : totalVar <= 10 ? "text-amber-600" : "text-red-600"
                  }`}>
                    {fmtPercent(totalVar)}
                  </span>
                </td>
